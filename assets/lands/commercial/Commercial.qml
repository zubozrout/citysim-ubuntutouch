...
Item {
    ...
    function getBuildings(level) {
		...
		var levelThree = [
			{"name" : "building_level3_a.png", "daysToSurvive": 365 * 40},
			{"name" : "building_level3_b.png", "daysToSurvive": 365 * 60}
		];
		switch(level) {
			case 1:
				return levelOne;
    ...
    function update(gameHolder, field) {
		if(objData.level === 0) {
			objData.boardImage = baseDir + "commercial.png";
